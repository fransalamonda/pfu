    ...
    function list_data()
    {
        if ($this->request->isAJAX()) {
            try {
                ...
                if ($plant_dept == 'IDCGALL') {
                    $str_plant = "<option value=''> - Select Option - </option>";
                    foreach ($respon_data as $d) {
                        if ($d['active'] == 'Y') {
                            $str_plant .= "<option required='' value='" . $d['id'] . "'>";
                            $str_plant .= $d['department_pic'] . "</option>";
                        }
                    }
                } else {
                    $str_plant = "<option value='" . $role_name . "'>'" . $role_name . "'</option>";
                }
    ...
    function view_action_pending()
    {
        if ($this->request->isAJAX()) {
            try {
                ...
                $valid3 = $this->validate(['st' => ['rules' => 'required', 'label' => 'Work Status', 'errors' => ['required' => 'Rules.st.required',]]]);
                if (!$valid3) {
                    $msg = ['errors' => ['errorID' => $validation->getError('st')]];
                }
                $id = $this->request->getVar('id');
                $tbl = $this->request->getVar('tbl');
                $ws = $this->request->getVar('st');
                ...
                $str_status = "<option value=''> - Select Option - </option>";
                foreach ($respon_work as $s) {
                    $_arr_st[$s['id']] = $s['work_status'];
                    if ($s['id'] == '1' or $s['id'] == '2') {
                        // if ($s['id'] == $ws) {
                        //     $str_status .= "<option value='" . $s['id'] . "'> ";
                        // } else {
                        //     $str_status .= "<option value='" . $s['id'] . "'>";
                        //     $str_status .= $_arr_st[$ws] . "</option>";
                        // }
            ...
            try {
                ...
                foreach ($respon_data as $item) {
                    ...
                    if ($item['Equipment_ID'] == '001_BC1') {
                        $nostr1++;
                        $str1 .= "<tr class='font-tabel-tr' >";
                        $str1 .= "<td   title=''><div class='font-tabel-btn'>Need Action</div></td>";
                        $str1 .= "<td   title=''>" . $newDate . "</td>";
                        $str1 .= "<td   title=''>" . $item['Equipment_ID'] . "</td>";

                        $str1 .= "<td   title=''>" . $item['Condition'] . "</td>";
                        $str1 .= "<td   title=''></td>";
                        $str1 .= "<td  class=''><a href='javascript:void(0)' " . $tmped . " title='Open Case'><h7 class='mt-3 mb-0 ' ><small></small>Open Case</h7></a></td>";
                        $str1 .= "</tr>";
                    }
                    if ($item['Equipment_ID'] == '001_BC11') {
                        $nostr2++;
                        $str2 .= "<tr class=' font-tabel-tr' >";
                        $str2 .= "<td  class='text' title=''><div class='font-tabel-btn'>Need Action</div></td>";
                        ...
                        $str2 .= "<td  class=''><a href='javascript:void(0)' " . $tmped . " title='Update Case'><h7 class=' ' ><small></small>Update Case</h7></a></td>";
                        $str2 .= "</tr>";
                    }
                ...
                if ($nostr3 == '0') {
                    $str3 = "<tr><td colspan='6' style='margin-left: auto;
                    margin-right: auto;'>";
                    $str3 .= "<div class='table-no-data'></Br>";
                    $str3 .= "<img class='table-no-data-icon' 
                                src='" .   $basurl . "/public/assets/images/Vector_ceklist.png' height='18'></Br>";
                ...
                $wfa_btn = "Waiting For Assign (<a>" . $nostr1 . "</a>)";
                $inp_btn = "In Progress (<a>" . $nostr2 . "</a>)";
                $nrw_btn = "Need Review (<a>" . $nostr3 . "</a>)";
